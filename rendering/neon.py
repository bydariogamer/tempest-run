...
class NeonLine:
    ...
    def __init__(
        ...
        self.np_points = numpy.array(
            [[round(p.y), round(p.x)] for p in points], numpy.int32
        ).reshape((-1, 1, 2))
        self.width = width
...
class NeonRenderer:
    ...
    def draw_lines(
        ...
        lines: Iterable[NeonLine],
        extra_darkness_factor=1,
    ):
        ...
        if not config.Debug.use_neon:
            for line in lines:
                pygame.draw.lines(
                    surface, line.color, False, line.vector_points, width=line.width
                )
            return

        if (
            self._buf is None
            or (self._buf.shape[0], self._buf.shape[1]) != surface.get_size()
        ):
        ...
        for line in lines:
            self.polylines(
                self._buf, [line.np_points], False, line.color, line.inner_width
            )
...
if __name__ == "__main__":
    ...
    for _ in range(N):
        lines.append(
            NeonLine(
                [rand_pt(), rand_pt()], random.randint(1, 1), random.choice(ALL_COLORS)
            )
        )
    ...
    renderer = NeonRenderer(
        ambient_bloom_kernel=(big_bloom, big_bloom),
